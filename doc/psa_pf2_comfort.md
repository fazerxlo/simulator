...
*   **Signals (Verified on 407 Log/Consistent Interpretation):**
    ...
    *   **Observed startup note:** In the ignition-on dump, `0x036` arrives as `0E 00 00 2A 01 00 00 A0`, which matches `ignition_on` and `dash_lights=1` with `lum=10`.
    *   **Observed companion message:** The same startup sequence includes a repeating `0x217` payload of `A2 00 80 00 00 FF FF E0`. This message appears immediately after ignition-on and is likely a BSI status broadcast; the simulator now displays it as raw monitor output.
...
*   **Period:** ~100ms
*   **CDCEmu Function (Receiver):** `processFrame` (Decodes Fan, AC, Recirc, Temps, and flags)
*   **Canbox Proxy Role (Receiver):** Use `peugeot_407_ms_1D0_climate_handler`.
*   **Signals (Verified from workbench dump):**
    *   Byte 2: Fan speed raw value. Valid values are `0-8`; `0x0F` appears as a transient/invalid marker and should not be treated as a new speed.
    *   Byte 3: Status bitmask. In dump data this byte is often `0x55`; it does not reliably encode direction by itself.
    *   Byte 4: Climate flags. `Bit 5` appears to be recirculation, `Bit 4` appears to be unfrost front.
    *   Byte 5: Left temperature raw value.
    *   Byte 6: Right temperature raw value.
*   **Note:** `0x1D0` is a primary HVAC status packet. Use it together with `0x1E3` for full fan/direction feedback.

---
**CAN ID:** `0x1E3`

*   **Source:** Climate Control ECU / EMF feedback.
*   **Description:** Secondary HVAC feedback message carrying fan state, direction, and mode bits.
*   **Period:** ~100ms
*   **Signals (Verified from workbench dump):**
    *   Byte 0: `AUTO` flag in bit 3, `DUAL` flag in bit 0.
    *   Byte 1: `unfrost front` flag in bit 7.
    *   Byte 4: left direction bits in the high nibble.
    *   Byte 5: right direction bits in the high nibble.
    *   Byte 6: Fan speed raw value. Valid values are `0-8`; `0x0F` appears as a transient/invalid marker.
    *   Byte 2: Left temperature raw value.
    *   Byte 3: Right temperature raw value.
*   **Note:** `0x1E3` is often the most reliable source for HVAC direction and mode state when using real ECU traffic.
*   **Observed high-nibble values:** `0x00`, `0x20`, `0x30`, `0x40`, `0x50`, `0x60`, `0x80`.
    *   `0x80` appears during automatic airflow distribution in Auto mode and does not directly map to a single manual direction button.
*   **Period:** Event-driven
*   **Signals:**
...
*   **Period:** ~100ms
*   **Proxy Role:** Read status and map to serial protocol AC messages (RZC `0x21` / MQB `0x21`).
*   **Signals:**
    *   Byte 2: Fan speed raw value (0-8 valid, `0x0F` is observed as an invalid/retain marker).
    *   Byte 4: Recirculation and unfrost-front flags.
    *   Byte 5: Left temperature raw value.
    *   Byte 6: Right temperature raw value.

**CAN ID:** `0x1E3`
*   **Source:** Climate Control ECU / EMF feedback.
*   **Description:** Secondary climate feedback message carrying fan, direction, and mode bits.
*   **Period:** ~100ms
*   **Signals:**
    *   Byte 0: `AUTO` and `DUAL` flags.
    *   Byte 1: `unfrost front` flag.
    *   Byte 4: left direction bits in the high nibble.
    *   Byte 5: right direction bits in the high nibble.
    *   Byte 6: Fan speed raw value (0-8 valid, `0x0F` is observed as invalid/retain marker).
    *   Byte 2/3: left/right temperature raw values.

*   **Note:** The simulator should use both `0x1D0` and `0x1E3` to reconstruct HVAC state accurately from real ECU traffic.
