---
name: CAN Sniff
description: "Use when working with CAN sniffing, vcan0 or can0 capture, baseline vs action comparison, candump log analysis, or identifying which CAN ID changes after a user action."
tools: [read, search, execute]
model: "GPT-5 (copilot)"
argument-hint: "Describe the signal or action to identify, plus the interface if not vcan0"
user-invocable: true
---
...
- Prefer `vcan0` unless the user explicitly asks for another interface.
...
1. Confirm the interface, defaulting to `vcan0`.
...
```bash
python -m tools.can_sniff_ai_agent sniff 5 --interface vcan0
python -m tools.can_sniff_ai_agent identify "hazard lights" --duration 5 --interface vcan0
...
```bash
.venv/bin/python -m tools.can_sniff_ai_agent identify "hazard lights" --duration 5 --interface vcan0
```
