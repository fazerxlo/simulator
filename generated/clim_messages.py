...
class Msg12D(CanMessage):
    ...
    def encode(self, car) -> list | None:
        if not car.bsi.ignition_on:
            return None
        comp_status = 0x80 if (car.clim.enabled and car.clim.ac) else 0x00
        return [comp_status, 0x32, 0x32, 0x00, 0x00, 0x00, 0x98, 0x80]
...
class Msg1D0(CanMessage):
    ...
    def encode(self, car) -> list:
        ...
        # Byte 4 bit layout (workbench-verified & RT4 RE):
        ...
        #   bit0 (0x01): "rear defrost" (REAR_DEFROST) — set when unfrost_rear is active.
        # AUTO mode: byte4=0x00; Fresh explicit: 0x20; Recirc explicit: 0x30.
        b4 = (0x20 if clim.intake_explicit else 0x00) | (0x10 if clim.recycle else 0x00) | (0x01 if clim.unfrost_rear else 0x00)
    ...
    def decode(self, car, data: bytes) -> None:
        ...
        # bit4 = recirculation (workbench-verified); bit5 = non-auto intake flag; bit0 = rear defrost.
        car.clim.recycle = (data[4] >> 4) & 1
        car.clim.unfrost_rear = data[4] & 1
