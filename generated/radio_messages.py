"""Auto-generated from signal-db/radio.yaml — do not edit by hand.
...
class Msg3E5(CanMessage):
    """Radio / head-unit front fascia buttons.
    Carries momentary button states for WIP Nav+ / RT6 front panel controls.
    """
    ...
    def encode(self, car) -> list | None:
        if not getattr(car.radio, 'active', False):
            return None
        p = car.radio.panel
        b0 = (p.get('menu', 0) << 6) | (p.get('tel', 0) << 4) | p.get('clim', 0)
        b1 = (p.get('trip', 0) << 6) | (p.get('mode', 0) << 4) | p.get('audio', 0)
        b2 = (p.get('ok', 0) << 6) | (p.get('esc', 0) << 4)
